  ...
  try {
    ...
    for (let i = 0; i < slots.length; i++) {
      ...
      if (!Number.isInteger(classId) || !Number.isInteger(lecturerId)) {
        errors.push(`Slot ${i + 1}: classId and lecturerId are required`);
        continue;
      }
      ...
      // Validate lecturer is assigned to the class's course
      ...
      const lecturerCourse = await prisma.lecturerCourse.findUnique({
        where: { lecturerId_courseId: { lecturerId, courseId: cls.courseId } },
      });
      if (!lecturerCourse) {
        errors.push(
          `Slot ${i + 1}: Lecturer is not assigned to course ${cls.course.code}. Assign the lecturer to this course first.`
        );
      }
    }
    ...
    const created = await prisma.$transaction(
      slots.map((s: { classId: number; lecturerId: number; dayOfWeek: string; shift: string; startTime: string; endTime: string; semester: string; year: number; room?: string }) =>
